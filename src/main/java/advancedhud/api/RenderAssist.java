...
public class RenderAssist {
    ...
    public static void drawUnfilledCircle(float posX, float posY, float radius, int num_segments, int color) {
        ...
        for (int i = 0; i < num_segments; i++) {
            ...
            tessellator.getWorldRenderer().pos(x + posX, y + posY, 0.0D).endVertex();// output vertex
    ...
    public static void drawUnfilledRect(float x1, float g, float x2, float y2, int color) {
        ...
        GL11.glColor4f(f1, f2, f3, f);
        tessellator.getWorldRenderer().begin(GL11.GL_LINE_LOOP, DefaultVertexFormats.POSITION);
        tessellator.getWorldRenderer().pos(x1, y2, 0.0D).endVertex();
        tessellator.getWorldRenderer().pos(x2, y2, 0.0D).endVertex();
        tessellator.getWorldRenderer().pos(x2, g, 0.0D).endVertex();
        tessellator.getWorldRenderer().pos(x1, g, 0.0D).endVertex();
        tessellator.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
    }
    ...
    public static void drawTexturedModalRect(float x, float y, float u, float v, float width, float height) {
        ...
        tessellator.getWorldRenderer().pos(x + 0, y + height, RenderAssist.zLevel).tex((u + 0) * f, (v + height) * f).endVertex();
        tessellator.getWorldRenderer().pos(x + width, y + height, RenderAssist.zLevel).tex((u + width) * f, (v + height) * f).endVertex();
        tessellator.getWorldRenderer().pos(x + width, y + 0, RenderAssist.zLevel).tex((u + width) * f, (v + 0) * f).endVertex();
        tessellator.getWorldRenderer().pos(x + 0, y + 0, RenderAssist.zLevel).tex((u + 0) * f, (v + 0) * f).endVertex();
        tessellator.draw();
    }
    ...
    public static void drawRect(float g, float h, float i, float j, int color) {
        ...
        tessellator.getWorldRenderer().pos(g, j, 0.0D).endVertex();
        tessellator.getWorldRenderer().pos(i, j, 0.0D).endVertex();
        tessellator.getWorldRenderer().pos(i, h, 0.0D).endVertex();
        tessellator.getWorldRenderer().pos(g, h, 0.0D).endVertex();
        tessellator.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_BLEND);
    }
    ...
    public static void renderInventorySlot(int slot, int x, int y, float partialTick, Minecraft mc) {
        RenderItem itemRenderer = new RenderItem(mc.getTextureManager(), getModelManager(mc));
        ...
        if (itemstack != null) {
            ...
            itemRenderer.renderItemOverlays(mc.fontRendererObj, itemstack, x, y);
        }
    }

    private static ModelManager getModelManager(Minecraft mc) {
        try {
            Field field = Minecraft.class.getDeclaredField("field_175617_aL");
            field.setAccessible(true);
            return (ModelManager) field.get(mc);
